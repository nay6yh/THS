...
def simulate_ths_grid_B(
    ...
    *,
    ...
    # ---- Phase B Engine Supervisor (optional) ----
    # If provided, Phase B will select among multiple base-step candidates to enforce:
    #   (1) start penalty
    #   (2) min ON/OFF hysteresis
    engine_gate: Optional[Dict[str, Any]] = None,
    ...
    # Engine gate params/state (kept in Phase B state dict; not passed to Phase A solver)
    gate_enabled = engine_gate is not None
    if gate_enabled:
    ...
    for idx, wltc_row in enumerate(wltc.to_dict(orient='records')):
        ...
        # Solve Phase A (injected) - FREE candidate
        base_free = solve_step_base_fn(x, **solver_kwargs)

        # Phase B Engine Supervisor: select base step candidate (Phase B-only)
        base = base_free
        start_event = False
        if gate_enabled:
            from .engine_gate_B import apply_engine_supervisor_B
            base, start_event = apply_engine_supervisor_B(
                x=x,
                base_free=base_free,
                solve_step_base_fn=solve_step_base_fn,
        ...
        # (1) Start fuel penalty reflected into actual fuel accounting (append-only behavior, no new columns)
        # This keeps fuel_balance_resid exact because we increase P_fuel and loss_engine equally.
        if gate_enabled and start_event:
            try:
                ...
                if dt_s > 0.0 and gate_params is not None and gate_params.start_fuel_g > 0.0:
                    ...
                    out["loss_engine_W"] = float(out.get("loss_engine_W", 0.0)) + P_add
            except Exception:
                pass
