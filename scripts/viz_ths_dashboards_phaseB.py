...
def _load_phaseB_timeseries_prefer_derived(path: Path) -> pd.DataFrame:
    """
    JP:
      - canonical(85列)の契約は変更しない。
      - viz側は derived(89列) が存在すればそれを優先して読み込む。
      - derived が無ければ、B.1b をオンザフライで計算して derived view を返す。

    EN:
      - Keep canonical 85-col contract untouched.
      - Prefer loading derived 89-col CSV when present.
      - Otherwise compute derived columns on-the-fly and return a derived view.

    Naming convention:
      canonical: timeseries_phaseB_<stamp>.csv
      derived:   timeseries_phaseB_derived_<stamp>.csv
    """
    p = Path(path)

    # If user already passed a derived file, just read it.
    if p.stem.startswith("timeseries_phaseB_derived_"):
        return pd.read_csv(p)

    derived_path = None
    if p.stem.startswith("timeseries_phaseB_"):
        stamp = p.stem[len("timeseries_phaseB_"):]
        derived_path = p.with_name(f"timeseries_phaseB_derived_{stamp}{p.suffix}")

    if derived_path is not None and derived_path.exists():
        return pd.read_csv(derived_path)

    # Fallback: read canonical and compute derived columns if missing
    ts = pd.read_csv(p)
    if ("fuel_cum_g" in ts.columns) and ("E_fuel_cum_J" in ts.columns):
        return ts

    ts2 = compute_phaseB_derived_fuel_integrals(
        ts,
        fuel_density_kg_per_L=None,  # set e.g. 0.745 if you want L/Lph columns too
        copy=True,
    )
    validate_phaseB_derived_fuel_integrals(ts2)
    return ts2
...
def _plot_fuel_cumulatives(ts: pd.DataFrame, outdir: Path, prefix: str, dpi: int = 160) -> None:
    # JP: derived列が無ければ何もしない
    if not {"t_s", "fuel_cum_g", "E_fuel_cum_J"}.issubset(ts.columns):
        return

    _ensure_dir(outdir)
    t = ts["t_s"].to_numpy(dtype=float)
    fuel_g = ts["fuel_cum_g"].to_numpy(dtype=float)
    E_MJ = ts["E_fuel_cum_J"].to_numpy(dtype=float) / 1e6

    fig = plt.figure(dpi=dpi)
    ax = fig.add_subplot(111)
    ax.plot(t, fuel_g, label="fuel_cum_g")
    ax.set_xlabel("t_s")
    ax.set_ylabel("Fuel cumulative [g]")
    ax.grid(True, alpha=0.3)
    ax2 = ax.twinx()
    ax2.plot(t, E_MJ, label="E_fuel_cum_MJ", linestyle="--")
    ax2.set_ylabel("Fuel energy cumulative [MJ]")
    fig.tight_layout()
    fig.savefig(outdir / f"{prefix}_fuel_cum.png")
    plt.close(fig)
...
def plot_energy_flow(ts: pd.DataFrame, outdir: Path, dpi: int = 160) -> None:
    ...
    for k in [
        ...
        "distance_km",
        "fuel_g_per_km","fuel_g_per_100km",
        "bsfc_implied_mean_g_per_kWh",
        "eta_eng_mean",
    ]:
        if k not in d:
            continue
...
def run_all(timeseries: str, outdir: str, dpi: int = 160) -> None:
    out = Path(outdir)
    _ensure_dir(out)
    ts = _load_phaseB_timeseries_prefer_derived(Path(timeseries))
    plot_overview(ts, out / "01_overview", dpi=dpi)
    plot_energy_flow(ts, out / "02_energy_flow", dpi=dpi)
    _plot_fuel_cumulatives(ts, out / "03_fuel_cum", prefix="03", dpi=dpi)
